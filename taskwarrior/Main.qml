...
Item {
            ...
            onStreamFinished: {
                ...
                try {
                    ...
                    for (var i = 0; i < tasks.length; i++) {
                        ...
                        if (task.start && task.status === "pending")
                            active = task;
                    }
    ...
    function processNextAction() {
        if (root.actionRunning || root.actionQueue.length === 0)
            return;
    ...
    function addTask(description, project, priority, due, tags) {
        ...
        runAction(cmd, pluginApi?.tr("main.task-added") || "Task added", pluginApi?.tr("main.error-add-failed") || "Failed to add task");
    }

    function completeTask(uuid) {
        if (!root.taskwarriorAvailable || !uuid)
            return;
        if (!isValidUuid(uuid))
            return;
        runAction(["task", String(uuid), "done"], pluginApi?.tr("main.task-completed") || "Task completed", pluginApi?.tr("main.error-complete-failed") || "Failed to complete task");
    }
    ...
    function deleteTask(uuid) {
        ...
        if (!isValidUuid(uuid))
            return;
        runAction(["task", "rc.confirmation=off", String(uuid), "delete"], pluginApi?.tr("main.task-deleted") || "Task deleted", pluginApi?.tr("main.error-delete-failed") || "Failed to delete task");
    }

    function modifyTask(uuid, field, value) {
        if (!root.taskwarriorAvailable || !uuid || !field)
            return;
        if (!isValidUuid(uuid))
            return;
        ...
        runAction(cmd, pluginApi?.tr("main.task-modified") || "Task modified", pluginApi?.tr("main.error-modify-failed") || "Failed to modify task");
    }
    ...
    function startTask(uuid) {
        ...
        if (!isValidUuid(uuid))
            return;
        runAction(["task", String(uuid), "start"], pluginApi?.tr("main.task-started") || "Task started", pluginApi?.tr("main.error-start-failed") || "Failed to start task");
    }
    ...
    function stopTask(uuid) {
        ...
        if (!isValidUuid(uuid))
            return;
        runAction(["task", String(uuid), "stop"], pluginApi?.tr("main.task-stopped") || "Task stopped", pluginApi?.tr("main.error-stop-failed") || "Failed to stop task");
    }
